...
<?php $update_my_posts = mysqli_query($db, "UPDATE admin SET exco_views='$exco_count' WHERE id=$admin_id"); ?>
<?php
...
if (isset($_POST['submit'])) {
	...
	$message = $_POST['message'];
		...
		$sql = mysqli_query($db, "INSERT INTO  excochat SET picture='$picture', uname='$uname', status='1', posts='$message', date=NOW()");
		if($sql) {
         ?>
		   <script type="text/javascript">
//		alert("Post Sent ");
		window.location="excochat.php";																	//header location
			</script>
		<?php
...
$query = "SELECT * FROM excochat WHERE status='1' ORDER BY id ASC ";
...
?>
		<div class="chat self" id="self">
			...
			<p class="chat-message"><?php echo $row['posts']?></p>
		</div>
<?php
	}else{
...
<form name="contact-form" method="post" action="excochat.php">
        <div class="chat-form">
